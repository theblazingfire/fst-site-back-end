...
io.on("connection", (socket) => {
  ...
  socket.on('user_online', async (userId) => {
    try {
      logger.infoLogger(`User Online Event - userId: ${userId}, socketId: ${socket.id}`);
      ...
      if (updateResult.nModified > 0) {
        ...
        logger.infoLogger(`No user found or socket ID already up-to-date for user ${userId}`);
      }
    } catch (err) {
      logger.errorLogger(`Failed to update socket ID for user ${userId}: ${err.message}`);
    }
  });
